...
export default function ImportPlayPage() {
    ...
    const [stagedProcessedData, setStagedProcessedData] = useState<any>(null);
    ...
    return (
        <main className="min-h-screen bg-black text-white">
            ...
            {/* Main Content: Either Form or Report */}
            <div className="max-w-6xl mx-auto px-6 py-8">
                {showReport ? (
                    <ProcessingReport
                        processedData={stagedProcessedData}
                        isSaving={isSaving}
                        error={error}
                        onBack={() => {
                            setShowReport(false);
                            setShowProcessingWizard(true);
                        }}
                        onSave={async () => {
                            setIsSaving(true);
                            setError(null);
                            try {
                                const payload = {
                                    ...
                                    year, // PERSIST YEAR
                                    theme,
                                    description: `Processed Scenario (${stagedProcessedData.frames.length} frames)`,
                                    source_text: rawText,
                                    is_processed: true,
                                    processed_data: stagedProcessedData
                                };
                                ...
                                if (!res.ok) {
                                    throw new Error(data.details || 'Failed to save to database');
                                }

                                router.push('/play-library');
                            } catch (err: any) {
                                console.error('[SAVE-ERROR]', err);
                                setError(err.message || 'Failed to save processed script.');
                                setIsSaving(false);
                            }
                        }}
                    />
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            {/* ... (Existing form content omitted for brevity in replacement, but I will keep it) ... */}
                            {/* 1. Metadata */}
            ...
            {showAIValidator && (
                <AIScriptValidator
                    ...
                    onValidated={(structure) => {
                        ...
                        if (structure.metadata.year) setYear(structure.metadata.year);
            ...
            {showProcessingWizard && validatedScript && (
                <ProcessingWizard
                    validatedStructure={validatedScript}
                    initialConfig={processingConfig}
                    onComplete={async (config) => {
                        setProcessingConfig(config);
                        setShowProcessingWizard(false);

                        // Map structure to processed data format
                        const colors = ['#3b82f6', '#ef4444', '#10b981', '#f59e0b', '#8b5cf6'];
                        const processedData = {
                            id: title.toLowerCase().replace(/[^a-z0-9]+/g, '_'),
                            title,
                            ...
                            frames: validatedScript.acts.flatMap((act: any, actIdx: number) =>
                                act.scenes.flatMap((scene: any, sceneIdx: number) =>
                                    scene.lines
                                        .filter((line: any) => line.type === 'dialogue')
                                        .map((line: any, lineIdx: number) => {
                                            const charConfig = config.characters.find(
                                                (c: any) => c.name === line.character
                                            );
                                            return {
                                                name: `Act ${actIdx + 1}, Scene ${sceneIdx + 1}, Line ${lineIdx + 1}`,
                                                description: line.text.substring(0, 60),
                                                character: line.character,
                                                voiceId: charConfig?.voiceId || 'rachel',
                                                trauma: charConfig ? charConfig.psychometrics.vulnerability / 100 : 0.5,
                                                entropy: charConfig ? charConfig.psychometrics.aggression / 100 : 0.5,
                                                focusLayer: lineIdx % 7,
                                                script: {
                                                    speaker: line.character,
                                                    text: line.text,
                                                    voiceId: charConfig?.voiceId || 'rachel',
                                                    previewUrl: charConfig?.previewUrl,
                                                    chord: config.generateChords ? 'AI-Generated' : 'TBD',
                                                    analysis: config.computePsychometrics ? 'Computed' : 'Pending'
                                                }
                                            };
                                        })
                                )
                        ...
                        setStagedProcessedData(processedData);
                        setShowReport(true);
                    }}
                    onCancel={() => {
                        setShowProcessingWizard(false);
                        setShowAIValidator(true); // Go back to validator
                    }}
                />
            )}
        </main>
